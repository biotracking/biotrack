...
{
  IplImage *src_tempimage = cvLoadImage("/home/pipeihuang/ABOS/tools/prosilica2file/output(video and timestamp)/template.jpg",1);
  if(src_tempimage == 0 )
    {
...
void Background::run()
{
  ...
  //////////////////////////////////////////////////////////
  //////////////////////////////////////////////////////////
  int record_frame = 300;
  int frameW  = 640; //  for firewire cameras
  int frameH  = 480; //  for firewire cameras
  int isColor = 1;
  string str_outVideo(outFolder);
  str_outVideo+="/out.avi";
  CvVideoWriter *writer = cvCreateVideoWriter(str_outVideo.c_str(),CV_FOURCC('P','I','M','1'),
					      fps,cvSize(frameW,frameH),isColor);
  string str_outTimeFile(outFolder);
  str_outTimeFile+="/ts.txt";
  ofstream outTimeFile(str_outTimeFile.c_str(), ios_base::out);
  ...
  while(!stopped ){
	...
	bgSet = false;
      ...
      ///////////// //record the video and timestamp in the HDD;
     // if(srcFrame->getFrameNumber()<=300){
     //   count++;
     //   std::cout<<"output_count = "<<count<<"   frame NO = "<<srcFrame->getFrameNumber()<<std::endl;
     //   //get the specific timestamp for each frame;
     //   if(srcFrame->getFrameNumber() != PreFrameNumber ){  
     // 	 timespec ts;
     // 	 clock_gettime(CLOCK_REALTIME, &ts);
     // 	 std::cout<<"timestampe of currently recording frame = "<<ts.tv_sec<<"."<<ts.tv_nsec<<std::endl;
     // 	 outTimeFile<<ts.tv_sec<<"."<<ts.tv_nsec<<std::endl;	
     // 	 cvWriteFrame(writer,frame);      // add the frame to the file
     // 	 PreFrameNumber =srcFrame->getFrameNumber();
     //   }
     // }

     if(++count<=record_frame){
       std::cout<<"record_frame# = "<<count<<std::endl;
       //get the specific timestamp for each frame;
       timespec ts;
       clock_gettime(CLOCK_REALTIME, &ts);
       std::cout<<"timestampe = "<<ts.tv_sec<<"."<<ts.tv_nsec<<std::endl;
       outTimeFile<<count<<"  "<<ts.tv_sec<<"."<<ts.tv_nsec<<std::endl;	
       cvWriteFrame(writer,frame);      // add the frame to the file
      }


     //tracking algrithm,output the circle;
     // /////****
      if( !frame )
      	{
	...
	timespec ts;
	clock_gettime(CLOCK_REALTIME, &ts);
	outTimeFile_circle<<"  <row>  "<<"1:"<<ts.tv_sec<<"."<<ts.tv_nsec<<"  2:"<<track_box.center.x<<"  3:"<<track_box.center.y<<"  </row>"<<std::endl;	    
	if(count_circle == record_frame)
	 outTimeFile_circle<<"</data>"<<std::endl;
